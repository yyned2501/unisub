  ...
  >
    <div v-if="media" class="flex gap-4 items-start mb-4">
      <img
        ...
        class="w-22.5 h-32 rounded-lg object-cover shrink-0"
      ...
      <div class="flex-1 pt-1">
        ...
        <p class="text-xs opacity-50 mb-2">
          {{ media.year || '未知年份' }} · {{ media.media_type === 'tv' ? '剧集' : '电影' }}
        </p>
        <span
          v-if="media.tmdb_id"
          class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-xs font-mono"
          style="background: var(--n-action-color); opacity: 0.6"
        >
