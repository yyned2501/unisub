...
function onImgError(e: Event) {
  ...
  img.src =
    'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="200" height="300"><rect fill="%2327272a" width="200" height="300"/><text fill="%236b7280" font-size="30" x="50%" y="50%" text-anchor="middle" dy=".3em">?</text></svg>'
}
</script>
...
<template>
  <div class="media-card" @click="$emit('click', media)">
    <div class="relative w-full pb-[150%] rounded-lg overflow-hidden" style="background: var(--n-border-color)">
      ...
      <div
        v-if="subscribed"
        class="absolute bottom-1.5 left-1.5 w-7 h-7 rounded-full bg-black/50 flex items-center justify-center"
      >
    ...
    <div class="px-0.5 pt-1.5 text-center">
      ...
      <div class="flex items-center justify-between">
        ...
        <n-button v-if="!subscribed" size="tiny" secondary type="primary" @click.stop="$emit('subscribe', media)"
          >订阅</n-button
        >
