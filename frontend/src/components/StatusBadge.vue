...
type TagType = 'default' | 'error' | 'warning' | 'success' | 'info' | 'primary'
type TagInfo = { label: string; type: TagType }

const NF_STATUS_MAP: Record<string, TagInfo> = {
  active: { label: '活跃', type: 'primary' },
  missing_fill: { label: '缺集', type: 'warning' },
  completed: { label: '已完成', type: 'success' },
}

const SUB_STATUS_MAP: Record<string, TagInfo> = {
  subscribed: { label: '已订阅', type: 'success' },
  unsubscribed: { label: '未订阅', type: 'default' },
  completed: { label: '已完成', type: 'success' },
}

const GENERAL_STATUS_MAP: Record<string, TagInfo> = {
