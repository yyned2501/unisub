...
const { items, loading, movingIds, resolvingIds, targetPath, resolvedMap, loadList, handleResolve, confirmMove } =
  useTmdb404()
...
<template>
  <div>
    ...
    <p class="text-sm opacity-55 mb-2">
      Emby 中有缓存记录但 TMDB 中没有对应数据的剧集，通常是因为 Emby 刮削器分配了无效的 TMDB ID。 请先点击「确认路径」从
      CD2 验证实际文件夹位置，确认后再移动。
    </p>
    ...
    <p v-else class="text-xs text-yellow-500 mb-4">
      <i class="ri-alert-line"></i> 未配置 CD2 移动目标路径，请先在
      <router-link to="/settings/cd2" class="underline">CD2 设置</router-link> 中配置。
    </p>

    <n-spin :show="loading">
      <n-empty v-if="!loading && !hasItems" description="没有发现无 TMDB 数据的媒体" size="small" class="py-12" />

      <n-card v-if="hasItems" size="small" :bordered="true">
        <n-virtual-list :items="items" :item-size="96" style="max-height: calc(100vh - 260px)" key-field="tmdb_id">
          <template #default="{ item: s }">
            <div class="flex items-start gap-3 py-2 px-1" :style="{ borderBottom: '1px solid var(--n-border-color)' }">
              <!-- 海报 -->
              <img
                v-if="s.emby_image_url"
                :src="s.emby_image_url"
                :alt="s.emby_series_name"
