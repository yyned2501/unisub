...
const { form, loading, saving, testing, saveConfig, testConnection } = useCd2Settings()
</script>
      ...
      <n-card title="连接设置" size="small" class="max-w-2xl">
        ...
        <n-form label-placement="top" :model="form">
          <n-form-item label="URL">
            <n-input v-model:value="form.base_url" placeholder="例如：192.168.31.10:19798" />
          ...
          <n-form-item label="移动到的位置">
            <n-input v-model:value="form.target_path" placeholder="/115open/待整理/转存/" />
          ...
          <n-form-item label="Emby 路径前缀">
            <n-input v-model:value="form.path_prefix" placeholder="/media/Symedia/MoviePilot" />
            <template #feedback>Emby 中媒体路径的前缀部分。</template>
          </n-form-item>

          <n-form-item label="替换为 CD2 路径">
            <n-input v-model:value="form.path_replacement" placeholder="/115open/已整理" />
