...
async def fetch(options: dict) -> list[RankMediaItem]:
    ...
    Args:
        options: 配置字典，含 mikan_season, mikan_year, mikan_resolve_detail
...
async def _fetch_anime_list(year: int, season: str) -> list[RankMediaItem]:
    """从 Mikan 获取新番列表。"""
    base_url = await _find_available_base()
    if not base_url:
    ...
    result = await _fetch_text(url, headers=headers, params=params)
    if not result:
        return []

    # 解析 div.an-box 列表（新结构：span data-src + a.title）
...
async def _resolve_detail(item: RankMediaItem) -> None:
    """解析单部新番详情页，获取年份和 Bangumi ID。"""
    mikan_id = item.source_meta.get("mikan_id")
    if not mikan_id:
        return

    base_url = await _find_available_base() or "https://mikanani.me"
    ...
    result = await _fetch_text(url, headers=headers)
    if not result:
        return
...
async def _find_available_base() -> str | None:
    ...
    async with _available_base_lock:
        ...
        for url in MIKAN_BASE_URLS:
            try:
                text = await _fetch_text(url, headers={"user-agent": "Mozilla/5.0"})
...
async def _fetch_text(url: str, headers: dict | None = None, params: dict | None = None) -> str | None:
    """用 httpx GET 请求文本内容。"""
    try:
