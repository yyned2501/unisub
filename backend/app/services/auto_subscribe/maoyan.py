...
class MaoyanRankProvider(RankProvider):
    ...
    async def fetch_async(self, options: dict) -> list[RankMediaItem]:
        ...
        num = self.to_int(options.get("maoyan_num"), 10)

        if options.get("maoyan_movie_box", True):
            try:
                box_items = await self._fetch_movie_box(num)
                items.extend(box_items)
            except Exception as e:
                logger.warning(f"[猫眼] 票房榜抓取失败: {e}")

        platforms = self.as_list(options.get("maoyan_web_platforms"))
        media_types = self.as_list(options.get("maoyan_web_types"))

        if platforms and media_types:
            try:
                web_items = await self._fetch_web_heat(platforms, media_types, num)
    ...
    async def _fetch_movie_box(self, num: int) -> list[RankMediaItem]:
        """抓取实时票房榜（JSON API）。"""
        headers = self._headers()
        data = await self._fetch_json(f"{MAOYAN_URL}/dashboard-ajax/movie", headers=headers)
        if not data:
            return []

        movie_list = (data.get("movieList") or {}).get("list") or []
        items: list[RankMediaItem] = []
        for entry in movie_list[:num]:
    ...
    async def _fetch_web_heat(
        ...
        for platform in platforms:
            platform_code = PLATFORM_TYPE.get(platform)
            if platform_code is None:
                continue
            for media_type in media_types:
                series_code = SERIES_TYPE.get(media_type)
                if series_code is None:
                    continue
                try:
                    batch = await self._fetch_web_heat_one(series_code, platform_code, num, headers)
                    for item in batch:
